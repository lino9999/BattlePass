...
public class RewardsCategoryGui {
    ...
    public void open() {
        ...
        boolean english = isEnglishLanguage();
        int editingSeason = plugin.getRewardEditorManager().getSeasonEditingContext(player.getUniqueId());
        String seasonLabel = editingSeason > 0
                ? " &8[&e" + (english ? "Season " : "Сезон ") + editingSeason + "&8]"
                : "";

        String title = isPremium ?
                GradientColorParser.parse("<gradient:#FFD700:#FF6B6B>" + (english ? "★ Premium Rewards" : "★ Премиум-награды")
                        + "</gradient>" + seasonLabel + " &8- &7" + (english ? "Page " : "Страница ") + page) :
                GradientColorParser.parse("<gradient:#4ECDC4:#45B7D1>" + (english ? "⚡ Free Rewards" : "⚡ Бесплатные награды")
                        + "</gradient>" + seasonLabel + " &8- &7" + (english ? "Page " : "Страница ") + page);
        ...
        if (page > 1) {
            ...
            prevMeta.setDisplayName(GradientColorParser.parse("<gradient:#4ECDC4:#45B7D1>"
                    + (english ? "← Previous page" : "← Предыдущая страница") + "</gradient>"));
            List<String> prevLore = new ArrayList<>();
            prevLore.add(GradientColorParser.parse("&7" + (english ? "Go to page " : "Перейти на страницу ") + (page - 1)));
        ...
        int maxLevel = Math.max(
                plugin.getRewardManager().getMaxLevel(),
                rewardsByLevel.keySet().stream().mapToInt(Integer::intValue).max().orElse(0)
        );
        if (endLevel < maxLevel) {
            ItemStack nextPage = new ItemStack(Material.ARROW);
            ItemMeta nextMeta = nextPage.getItemMeta();
            nextMeta.setDisplayName(GradientColorParser.parse("<gradient:#4ECDC4:#45B7D1>"
                    + (english ? "Next page →" : "Следующая страница →") + "</gradient>"));
            List<String> nextLore = new ArrayList<>();
            nextLore.add(GradientColorParser.parse("&7" + (english ? "Go to page " : "Перейти на страницу ") + (page + 1)));
        ...
        backMeta.setDisplayName(GradientColorParser.parse("<gradient:#FF6B6B:#FF0000>"
                + (english ? "← Back" : "← Назад") + "</gradient>"));
        List<String> backLore = new ArrayList<>();
        backLore.add(GradientColorParser.parse("&7" + (english ? "Return to category selection" : "Вернуться к выбору категории")));
        ...
        saveMeta.setDisplayName(GradientColorParser.parse("<gradient:#00FF88:#45B7D1>"
                + (english ? "✓ Save and apply all" : "✓ Сохранить и применить всё") + "</gradient>"));
        List<String> saveLore = new ArrayList<>();
        saveLore.add(GradientColorParser.parse("&7" + (english ? "Save all changes and reload" : "Сохранить все изменения и перезагрузить")));
    ...
    private Map<Integer, List<Reward>> loadSeasonRewards(int seasonNumber, boolean premium) {
        ...
        String defaultRewardName = isEnglishLanguage() ? "Reward" : "Награда";
                ...
                if (items != null) {
                    for (String itemKey : items.getKeys(false)) {
                        ...
                        if (itemSection != null && itemSection.contains("command")) {
                            String display = itemSection.getString("display", defaultRewardName);
                            ...
                            String displayName = itemSection.getString("display-name",
                                    resolveMaterialDisplayName(material));
                            rewards.add(new Reward(level, "", displayName, premium));
                        }
                    }
                }
            } else if (section.contains("command")) {
                String display = section.getString("display", defaultRewardName);
    ...
    private String resolveMaterialDisplayName(String materialName) {
        return MaterialNameResolver.resolve(materialName, getCurrentLanguage());
    }
    ...
    private ItemStack createLevelChest(int level, List<Reward> rewards) {
        ...
        if (hasRewards) {
            lore.add(GradientColorParser.parse("<gradient:#4ECDC4:#45B7D1>"
                    + (english ? "Current rewards:" : "Текущие награды:") + "</gradient>"));
            for (Reward reward : rewards) {
                lore.add(GradientColorParser.parse("&7• &f" + reward.displayName));
            }
        } else {
            lore.add(GradientColorParser.parse("&7" + (english ? "Rewards are not configured" : "Награды не настроены")));
        }

        lore.add("");
        lore.add(GradientColorParser.parse("<gradient:#FFD700:#FF6B6B>"
                + (english ? "▶ CLICK TO EDIT" : "▶ НАЖМИТЕ ДЛЯ РЕДАКТИРОВАНИЯ") + "</gradient>"));
